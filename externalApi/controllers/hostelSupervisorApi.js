...
const searchHostelSupervisors = asyncHandler(async (req, res) => {
  const {
    userName,
    userEmail,
  ...
  if (hostelName) {
    try {
      ...
      if (hostel) {
        ...
        return res.json({ hostelSupervisors: [], page: 1, pages: 0, total: 0 })
      }
    } catch (error) {
  ...
  userQuery.role = "Hostel Supervisor" // Ensure we only query Hostel Supervisor users
  ...
  if (userName || userEmail) {
    ...
    // Optimization: If no specific user is searched, we can get all Hostel Supervisor User IDs
    // to potentially narrow down the HostelSupervisor.find later, though mongoose might optimize this.
    try {
      const hostelSupervisorUsers = await User.find({ role: "Hostel Supervisor" }).select("_id").lean()
      if (hostelSupervisorUsers.length === 0) {
        return res.json({ hostelSupervisors: [], page: 1, pages: 0, total: 0 })
      }
      // Add this condition only if other query conditions exist, otherwise it's redundant
      if (Object.keys(query).length > 0) {
        query.userId = { $in: hostelSupervisorUsers.map((u) => u._id) }
      }
    } catch (error) {
      console.error("Error finding hostel supervisor users:", error)
      res.status(500)
      throw new Error("Error finding hostel supervisor users")
    }
  }
  ...
  // --- Execution ---
  try {
    const count = await HostelSupervisor.countDocuments(query)
    const hostelSupervisors = await HostelSupervisor.find(query)
    ...
    console.error("Error searching hostel supervisors:", error)
    res.status(500)
    throw new Error("Server error during hostel supervisor search")
  }
})

export { searchHostelSupervisors } 